...
const StateMachine = opts => {
    ...
    let appMethods = {
        ...
        handleTx: txHandler => {
            if (!txHandler.route || !txHandler.handle) {
                throw new Error('Invalid transaction handler.')
            }

            txHandlers.push(txHandler)
        },
        ...
        compile(appState = {}) {
            ...
            const applyTx = (state, tx, context) => {
                ...
                try {
                    let txHandler = txHandlers.find(h => h.route === tx.route)

                    if (txHandler) {
                        txHandler.handle(txState, tx, context)
                    } else {
                        throw new Error(
                            `Unknown transaction route ${tx.route}.`
                        )
                    }
            ...
            const checkTransition = type => {
                if (!valid[prev].has(type)) {
                    throw Error(
                        `Invalid transition for params {type: ${type}, prev: ${prev}}.`
                    )
                }
